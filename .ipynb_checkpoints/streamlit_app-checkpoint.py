...
div[role="radiogroup"] > label:has(input:checked) { border-color: #2563eb; background: rgba(37, 99, 235, 0.08); }
...
st.markdown('<div style="text-align:center;"><h6>A steel plant energy & emissions model</h6</div>', unsafe_allow_html=True)
# right after the two st.markdown(...) title lines
tab_main, tab_sweep, tab_static = st.tabs(["Main Model", "Sensitivity", "Static Mods"])

# put your top metrics placeholder inside the Main tab
with tab_main:
    ...
    if MAP_PNG.exists():
        ...
        st.components.v1.html(
            ...
            <div style="display:flex;align-items:center;gap:8px;margin:0 0 8px 0;">
              ...
              <button title="Open full size map"
                      aria-label="Open full size map"
            ...
            <script>
              ...
              btn.addEventListener('click', () => {{
                ...
                w.document.title = 'Process flow map';
...
if run_now:
    with st.spinner("Running model (core)…"):
        route_cfg = RouteConfig(
            route_preset=route,                  # ← FORCED by scenario
            stage_key=stage_key,
            demand_qty=float(demand_qty),
        ...
        scn = ScenarioInputs(
            country_code=(country_code or None),
            scenario=scenario,                  # full scenario dict
            route=route_cfg,
        )
        ...
        # Pull internals (core reports these in gCO2/MJ)
        fi  = _dbg.get("f_internal")                       # fraction [0..1]
        efi = _dbg.get("ef_internal_electricity")          # gCO2/MJ (internal utility EF)
        cc  = _dbg.get("country_code") or (country_code or None)
        
        # Get grid EF from the same source the core uses: electricity_intensity.yml (gCO2/MJ)
        efg = None
        try:
            elec_map = load_electricity_intensity(os.path.join(DATA_ROOT, "electricity_intensity.yml")) or {}
        ...
        # Fallback: if electricity_intensity.yml doesn't have it, try emission_factors.yml 'Electricity' (assumed gCO2/MJ)
        ...
        # Mixed EF (all in gCO2/MJ)
        ef_mix = None
        if isinstance(fi, (int, float)) and isinstance(efi, (int, float)) and isinstance(efg, (int, float)):
            ef_mix = fi * efi + (1.0 - fi) * efg
        
        # # Display with correct units. Also show per-kWh in kgCO2/kWh.
        # def _to_kg_per_kwh(g_per_mj: float | None) -> str:
        #     if not isinstance(g_per_mj, (int, float)):
        #         return "—"
        #     kg_per_kwh = (g_per_mj * 3.6) / 1000.0
        #     return f"{kg_per_kwh:.3f}"
        
        # with st.sidebar:
        #     st.divider()
        #     st.caption("Debug — Electricity EFs (gCO₂/MJ)")
        
        #     c1, c2, c3, c4 = st.columns(4)
        #     with c1:
        #         st.metric("Country", cc or "—")
        #         st.metric("f_internal", "—" if not isinstance(fi, (int, float)) else f"{fi*100:.1f}%")
        #     with c2:
        #         st.metric("EF_grid", "—" if not isinstance(efg, (int, float)) else f"{efg:.2f}")
        #         st.caption(f"≈ {_to_kg_per_kwh(efg)} kgCO₂/kWh")
        #     with c3:
        #         st.metric("EF_internal", "—" if not isinstance(efi, (int, float)) else f"{efi:.2f}")
        #         st.caption(f"≈ {_to_kg_per_kwh(efi)} kgCO₂/kWh")
        #     with c4:
        #         st.metric("EF_mix", "—" if not isinstance(ef_mix, (int, float)) else f"{ef_mix:.2f}")
        #         st.caption(f"≈ {_to_kg_per_kwh(ef_mix)} kgCO₂/kWh")



    # Map core outputs to names the UI expects
    ...
    # st.write(f"**Stage material**: {STAGE_MATS[stage_key]}")
    ...
    # # ---- Sankey diagrams (optional, use core outputs) ----
