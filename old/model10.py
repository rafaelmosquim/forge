# -*- coding: utf-8 -*-
"""
Created on Wed Aug  6 18:41:05 2025
...
if __name__ == '__main__':
    ...
    energy_int      = load_data_from_yaml(os.path.join(base, 'energy_int.yml'))     # MJ/run
    ...
    #e_sh = derive_energy_shares(recipes, energy_content)


    # Build recipe lookup
    recipes_dict = {r.name: r for r in recipes} if recipes else {}
    print("DBG Coke recipe outputs →", recipes_dict['Coke Production'].outputs)
    print("DBG gas_per_run_cp →",
      recipes_dict['Coke Production'].outputs.get('Process Gas'))
    ...
    if not all((params, recipes, route_cfg, mkt_cfg)):
        ...
        if balance_matrix is None:
            ...
            # 7b) Repair Coke‐oven row back to its base intensity
            cp_runs = prod_lvl.get('Coke Production', 0.0)
            base_cp = params.coke_production_base_intensity
            cp_sh   = energy_shares['Coke Production']
            for c in energy_balance.columns:
                energy_balance.loc['Coke Production', c] = cp_runs * base_cp * cp_sh.get(c, 0.0)       
