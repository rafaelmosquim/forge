...
\begin{abstract}
Steel and aluminum production are energy-intensive industrial sectors with significant greenhouse gas (GHG) emissions. Accurate modeling of production routes, energy consumption, and emissions is essential for assessing decarbonization pathways and policy impacts. However, most existing models fall in one of two categories: detailed process-based LCA models that are complex and inflexible, or top-down sectoral models that lack process detail, usually behind proprietary data. FORGE aims to bridge this gap by providing and explicit, open source process-based techno-environmental model that is both flexible and user-friendly. It allows users to define and modify production routes, energy sources, and emission factors through simple YAML configuration files, enabling scenario analysis and sensitivity testing. Along with full transparency of inputs, outputs and parameters, FORGE is designed to be extensible to other industrial sectors beyond steel and aluminum. This document sketches the main architecture and---most importantly---the data pipeline from YAML input files to emissions outputs.
\end{abstract}
...
significant greenhouse gas (GHG) emissions. Together they account for roughly 10\% of global emissions. As such, accurate modeling of production routes, energy consumption, and emissions is essential for assessing decarbonization pathways and policy impacts. However, most existing models fall in one of two categories: detailed process-based LCA models that are complex and inflexible, or top-down sectoral models that lack process detail, usually behind proprietary data.
...
The model is implemented as a Python package with:
\begin{itemize}
...
reproduction difficult, and complicates boundary comparisons. It is also not suitable for scaling and extrapolation to new scenarios, routes, or sectors.
...
This document presents the main architecture and---most importantly---the data
pipeline from YAML input files to emissions outputs, as the basis of a full
methods paper.
...
The aluminum implementation now mirrors the original Excel model and runs through the same FORGE stack as steel: primary and remelt routes, downstream alloying, rolling/extrusion/ingot casting, finishing, and direct process emissions. Aluminum is therefore a first-class, fully integrated sector in FORGE; its process chain is somewhat shorter than steel’s and it is currently distributed with a single baseline dataset (minimum and maximum variants are planned).

\section{High-level Architecture}

\subsection{Steel Plant Overview}

\begin{figure}
      \centering
      \includegraphics[width=0.9\textwidth]{steel_process.png}
      \caption{Overview of the plant layout and process flow. Source: Own authors.}
\end{figure}

\subsection{Aluminum plant Overview}
\begin{figure}
      \centering
      \includegraphics[width=0.9\textwidth]{aluminum_process.png}
      \caption{Overview of the aluminum production process flow. Source: Own authors.}
\end{figure}
...
The gas routing and crediting logic follows the basic diagram below. Red lines are for direct use of process gas, while blue lines are for electricity generation via a utility plant. All purchases are in purple. 
\begin{center}
  \includegraphics[width=0.8\textwidth]{figs/gas_recovery.png}
\end{center}
...
Aluminum now mirrors the original Excel baseline and runs through the same pipeline as steel, using sector-specific descriptors and data:
\begin{itemize}
  \item \textbf{Routes and stages} are defined in
        \texttt{datasets/aluminum/baseline/sector.yml}, with stage definitions for
        primary, remelt, and finished aluminum (the UI defaults to the finished product menu).
...
aluminum currently has a single baseline dataset (minimum and maximum variants
are forthcoming).
...
\begin{itemize}
  ...
  \item sensitivity and Monte Carlo sweeps to ensure that the model responds
        ...
        point estimates.
\end{itemize}
Aluminum validation is more limited, reflecting both the sector’s narrower
process scope and the current single baseline dataset, but it exercises the
same pipeline end-to-end and provides a fully integrated second sector.
...
\subsection{Validation at the crude-steel boundary}

To validate FORGE against published ranges, we run each core steel route (BF--BOF, DRI--EAF, EAF--scrap) at the crude-steel boundary using the Likely dataset and Brazil grid electricity. Concretely, we call the live engine via \texttt{make validation-table}, which invokes \texttt{forge.steel\_core\_api\_v2.run\_scenario} for each route with \texttt{datasets/steel/likely}, \texttt{country=BRA}, demand of \SI{1000}{\kilogram} at the \texttt{Cast} stage (``Validation (as-cast)''). The resulting route-level emission factors are written to \texttt{results/validation\_routes\_bra.tex} and included below.

\begin{table}[H]
  \centering
  \caption{Route-level emission factors from FORGE at the crude-steel (as-cast) boundary, Likely dataset, Brazil grid electricity. Values shown are FORGE results only; comparison with Worldsteel and other benchmarks is discussed in the text.}
  \input{../results/validation_routes_bra.tex}
  \label{tab:validation_bra}
\end{table}

\subsection{Monte Carlo illustration}

Monte Carlo for the as-cast configuration, for Brazil, using mininum, likely and maximum datasets, shows the following results:

\begin{figure}
      \centering
      \includegraphics[width=0.9\textwidth]{mc-cast.png}
      \caption{Monte Carlo Simulation for cast steel in Brazil}
\end{figure}

\subsection{Paper portfolio scenarios}

Beyond single-route validation, FORGE includes a small scenario driver for sector-wide portfolio analysis. The \texttt{make paper} target runs \texttt{src/forge/scenarios/paper\_scenarios.py} with a ``paper'' portfolio specification (\texttt{configs/paper\_portfolio.yml}) and portfolio product configuration (\texttt{FORGE\_PAPER\_PRODUCT\_CONFIG=portfolio}). Internally, the script calls \texttt{run\_scenario} repeatedly across a grid of assumptions (utilization, annual efficiency improvement, charcoal expansion, DRI mixes) and aggregates emission intensities over time.

When invoked via \texttt{make paper}, output paths are configured automatically so that all figures are written under \texttt{results/paper/figs/} (e.g.\ \texttt{paper\_fig\_01.png}, \texttt{paper\_fig\_02.png}, \dots) and an aggregate emission-intensity table is written to \texttt{results/paper/tables/aggregate\_ef.csv}. The scenario results and figures used in this section of the paper are taken directly from those files; rerunning \texttt{make paper} regenerates them from the live FORGE model.
