...
def analyze_material_costs(matrix_data, mat_price: Dict[str, float], external_rows: Optional[Iterable[str]] = None) -> float:
    ...
    # Auto-extend with any explicit market/purchase rows present in the matrix.
    try:
        if hasattr(matrix_data, "index"):
            for row in matrix_data.index:
                row_s = str(row).strip().lower()
                if row_s == "external inputs" or " from market" in row_s or " purchase" in row_s:
                    if row not in external_purchase_rows:
                        external_purchase_rows.append(row)
    except Exception:
        pass
