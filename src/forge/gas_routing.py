"""
Gas routing and credit application utilities.

The public entry point (`apply_gas_routing_and_credits`) is intentionally pure:
callers supply the current energy balance, scenario metadata, and (optionally)
pre-computed reference metrics. The helper never reaches back into the core to
re-run scenarios, which keeps responsibilities clear and avoids circular imports.
"""
...
# Type alias for reference helper callbacks supplied by the caller.
ReferenceFn = Callable[..., float]


def _blend_ef(shares: Dict[str, float], emission_factors: Dict[str, float]) -> float:
    """Blend emission factors for a process based on carrier shares (skip electricity)."""
...
def _roles_for(process_roles: Dict[str, Iterable[str]], proc_name: str) -> set[str]:
    """Legacy helper to normalise role names for descriptor-defined processes."""
...
def apply_gas_routing_and_credits(
    ...
    """
    Adjust the energy balance, blend gas/electricity emission factors, and return
    diagnostics used by the API/UI.

    Args:
        energy_balance: Current energy balance dataframe.
        recipes: Active recipe objects (post overrides).
        prod_levels: Production runs per process.
        params: Scenario parameters namespace.
        energy_shares / energy_int / energy_content: Energy tables (post overrides).
        e_efs: Emission factors mapping (mutated copy returned).
        scenario: Metadata assembled by the API (gas config, roles, routing fractions).
        credit_on: Whether process-gas-to-electricity credits are applied.
        compute_inside_gas_reference_fn: Optional callback returning total plant gas MJ.
        compute_inside_elec_reference_fn: Optional callback returning plant electricity MJ.
    """
    ...
    # ---- Process gas volumes (legacy steel logic) ---------------------------------------
    try:
        coke_recipe = recipes_dict.get("Coke Production", Process("Coke Production", {}, {}))
        gas_coke_MJ = prod_levels.get("Coke Production", 0.0) * float(coke_recipe.outputs.get(process_gas_carrier, 0.0))
    except Exception:
        gas_coke_MJ = 0.0
    ...
    # ---- Reference metrics (total plant gas / electricity) ------------------------------
    ...
    # ---- Apply credits / modify energy balance ------------------------------------------
    adjusted_balance = energy_balance.copy()
    if credit_on:
        from forge.steel_model_core import adjust_energy_balance  # local import avoids circular dependency
        ...
        if direct_use_gas_MJ > 0 and total_gas_consumption_plant > 1e-9:
            ...
            for process_name in adjusted_balance.index:
                current_gas = float(adjusted_balance.loc[process_name, natural_gas_carrier])
                if current_gas <= 0:
                    continue
                reduction = current_gas * f_internal_gas
                adjusted_balance.loc[process_name, natural_gas_carrier] = current_gas - reduction
                adjusted_balance.loc[process_name, process_gas_carrier] += reduction
    else:
