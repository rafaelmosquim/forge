...
#
# The cache key previously covered only scenario arguments, so a change to the
# model source or to the dataset YAMLs left every cached value silently valid.
# Stale numbers could therefore survive arbitrary code and data changes. The
# fingerprint below hashes the forge package sources and the active dataset so
# any edit to either invalidates the cache.
# ---------------------------------------------------------------------------
...
def _compute_model_fingerprint() -> str:
    ...
    h = _hashlib.sha256()
    pkg_root = _Path(__file__).resolve().parent.parent          # .../forge
    ...
    if data_root.is_dir():
        for f in sorted(data_root.rglob("*")):
            if f.is_file() and f.suffix.lower() in {".yml", ".yaml", ".csv", ".json"}:
                try:
                    h.update(str(f.relative_to(data_root)).encode())
                    h.update(f.read_bytes())
                except Exception:
                    continue
